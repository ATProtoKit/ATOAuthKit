...
public struct OpenIDConnectUserInfo: Codable {
    ...
    /// The end user within the identity provider.
    ///
    /// This is how the application knows which user has authenticated.
    public let subject: String

    /// The unique identifier of the identity provider. Optional.
    ///
    /// This is so the client can identify who issued the token.
    public let issuer: URL?

    /// The intended recipient(s) for this token—usually, the client application's identifier (such as its OAuth client ID).
    ///
